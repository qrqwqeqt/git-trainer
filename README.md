...
## Безпека (модель загроз та захист даних)

**Модель довіри:** «довірена аудиторія» — навчальна група під керівництвом
викладача. Первинний захист — повна ізоляція виконання; особистість —
сервер-підписана, з аудитом і обмеженням частоти.

**Ізоляція виконання (основний рубіж):**
...
- Sandbox запускається з `network=none`, `cap_drop=ALL`, `--security-opt no-new-privileges`,
  `mem_limit=256m`, `pids_limit=128`, під непривілейованим користувачем (UID 1000).
- Усі команди проходять **whitelist** (`git status`, `commit`, `branch`, `merge`, `rebase`, …)
  + блокування shell-метасимволів (`;`, `|`, `&&`, `` ` ``, `$(`); решта — `ERROR`.

**Авторизація:**
- WebSocket приймає лише **підписаний токен сесії** (HMAC-SHA256, `POST /auth/session`).
  Особистість (`user_id`, `username`, `room`) видається сервером і не може бути підмінена
  на кожному кадрі; токен прив'язаний до кімнати та має строк дії.

**Аудит та доступність:**
- Кожна git-команда пишеться в **аудит-лог** (`command_audit`): хто, що, коли, exit-code.
  Перегляд — `GET /rooms/{slug}/audit`.
- **Rate-limiting** (sliding-window, 30 команд / 10 с на користувача) — захист від флуду/DoS.
- Усі помилки логуються з контекстом `room_id` + `user_id`; на рівні REST/WS — валідація Pydantic, shell не задіяний.

**Поза обсягом (майбутнє посилення):** пароль/ролі кімнати (викладач/студент),
наскрізне шифрування, повноцінні акаунти. Для відкритих кімнат особистість —
самозаявлена, але цілісно підписана й повністю проаудитована.
